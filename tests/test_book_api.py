...
class TestBookApi:
    ...
    def test_create_book_api(self, app):
        with app.test_client() as client:
            response: Response = client.post('/books/create', json={
                'title': 'Pnin',
            ...
            response: Response = client.get('/books?title=Pnin&author=Vladimir%20Nabokov')
            ...
            response: Response = client.get('/books?title=Demian&author=Hermann%20Hesse')
            assert response.status_code
            json_body = response.get_json(force=True)
